...
Codex ACP 버전: `0.142.1` · ACP 계약 구현: advertised `100%` (`13/13` handler), enabled SDK surface `13/16` (`session/fork`는 광고하지 않음; `mcp/connect` 비활성화)
...
- Codex 기반 ACP 세션: 생성·로드·재개·목록·삭제·종료·리플레이
...
ACP `session/fork`와 MCP-over-ACP `mcp/connect` proxy method는 아직 직접
노출하지 않습니다. 클라이언트가 세션 생성 시 넘기는 MCP 서버 설정은 Codex MCP
config로 변환하며, MCP tool 출력은 기존처럼 ACP tool call로 렌더링합니다.
