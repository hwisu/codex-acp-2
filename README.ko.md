...
- 클라이언트 제공 MCP 서버 (HTTP, stdio, MCP-over-ACP; SSE는 무시)와
...
thread를 시작합니다. MCP-over-ACP `mcp/connect`는 세션 단위 loopback
Streamable HTTP bridge로 노출합니다. ACP `type: "acp"` MCP 서버는
`mcp/connect`로 연결하고 `mcp/message`로 proxy한 뒤 세션 종료 시 disconnect합니다.
클라이언트가 세션 생성 시 넘기는 MCP 서버 설정은 Codex MCP config로 변환하며,
MCP tool 출력은 기존처럼 ACP tool call로 렌더링합니다.
