...
Codex ACP 버전: `0.142.1` · ACP 계약 구현: advertised `100%` (`14/14` handler), enabled SDK surface `14/16` (`session/fork` 활성화; `mcp/connect` 비활성화)
...
- Codex 기반 ACP 세션: 생성·로드·재개·포크·목록·삭제·종료·리플레이
- 텍스트·리소스·링크·이미지 프롬프트 블록
- 메시지·reasoning·tool call·백그라운드 상태 스트리밍
- 셸 명령 승인/출력 및 apply-patch 편집 렌더링
- 클라이언트 제공 MCP 서버 (HTTP, stdio; SSE는 무시)와
  `additionalDirectories` workspace root
...
ACP `session/fork`는 원본 rollout을 snapshot하여 새 세션 ID를 가진 독립 Codex
thread를 시작합니다. MCP-over-ACP `mcp/connect` proxy는 아직 노출하지 않습니다.
Codex가 현재 stdio/HTTP MCP transport를 받는 반면 ACP MCP는 live proxy bridge가
필요하기 때문입니다. 클라이언트가 세션 생성 시 넘기는 stdio/HTTP MCP 서버
설정은 Codex MCP config로 변환하며, MCP tool 출력은 기존처럼 ACP tool call로
렌더링합니다.
