...
Codex ACP 버전: `0.137.0` · ACP 계약 구현: advertised `100%` (`11/11` handler), enabled SDK surface `11/16` (`session/delete`, `session/fork`, `session/resume`은 광고하지 않음; `mcp/connect` 비활성화)
...
- Codex Rust crates: [`openai/codex`](https://github.com/openai/codex/tree/rust-v0.137.0/codex-rs)
  tag `rust-v0.137.0` (`Cargo.lock` 기준 `f221438b691b8f749d98f22077c93ebe01923fbe`)
- ACP Rust SDK: [`agent-client-protocol`](https://crates.io/crates/agent-client-protocol)
  ...
  `agent-client-protocol = 0.14.0` + `unstable`
  (`agent-client-protocol-schema = 0.13.6`, lockfile 기준)
